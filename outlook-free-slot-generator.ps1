<#
.SYNOPSIS
    Genera le disponibilità Outlook in formato Tabella HTML o Elenco Testuale con temi e font configurabili.
#>
Param(
...
# Selezione del Tema e dei Font
$themeName = $config.Preferences.SelectedTheme
$theme = $config.ColorThemes.$themeName
if (-not $theme) { $theme = $config.ColorThemes.Grigio } # Fallback

$fName = if ($config.Preferences.FontName) { $config.Preferences.FontName } else { "Calibri, sans-serif" }
$fSize = if ($config.Preferences.FontSize) { $config.Preferences.FontSize } else { "11pt" }
$outlookFont = "font-family: $fName; font-size: $fSize;"
...
# ---- 5. OUTPUT (DINAMICO E CONFIGURABILE) ----
...
if ($Formato -eq "Tabella") {
    ...
    foreach ($d in $daysProcessed) {
        ...
        $badges = foreach ($s in $d.Slots) {
            ...
            # Badge con font configurabile
            "<span style='display:inline-block;background:$($theme.BadgeBg);color:$($theme.BadgeText);border:1px solid $($theme.BadgeBorder);padding:2px 8px;border-radius:4px;margin:1px;font-weight:600;$outlookFont'>$range</span>"
        }
        
        $bg = if ($count++ % 2 -eq 0) { "#ffffff" } else { $theme.RowAlt }
        $rows += "<tr style='background:$bg;'><td style='padding:8px 15px;font-weight:bold;border-bottom:1px solid #e2e8f0;color:#333333;$outlookFont'>${dayStr}</td><td style='padding:8px 15px;border-bottom:1px solid #e2e8f0;$outlookFont'>$($badges -join ' ')</td></tr>"
    }

    $mailBody = @"
    $htmlHeader
    <div style="$outlookFont color:#000000; max-width:650px;">
        <p style="margin-bottom:15px;">$($txt.Greeting)</p>
        <p style="margin-bottom:15px;">$($txt.IntroText)</p>
        <table style="border-collapse:collapse; border:1px solid #e2e8f0; min-width:400px;">
            <thead>
                <tr style="background:$($theme.HeaderBg); color:$($theme.HeaderText);">
                    <th style="padding:10px 15px; text-align:left; font-size:0.9em; text-transform:uppercase; letter-spacing:0.05em;$outlookFont">$($txt.TableHeaderDay)</th>
                    <th style="padding:10px 15px; text-align:left; font-size:0.9em; text-transform:uppercase; letter-spacing:0.05em;$outlookFont">$($txt.TableHeaderAvailability)</th>
                </tr>
            </thead>
            <tbody>
                $rows
        ...
        <p style="margin-top:20px;">$($txt.Closing)</p>
    </div>
"@
} else {
    # Modalità Testo con font configurabile
    $lines = @("$htmlHeader<div style='$outlookFont color:#000000;'><div>$($txt.Greeting)</div><br><div>$($txt.IntroTextList)</div>")
    foreach ($d in $daysProcessed) {
        $dayStrRaw = $d.Day.ToString("dddd dd/MM", $culture)
        $dayStr = $dayStrRaw.Substring(0,1).ToUpper() + $dayStrRaw.Substring(1)
        ...
        $lines += "<div style='margin-bottom:5px;$outlookFont'><b>${dayStr}:</b> $($parts -join $txt.OrText)</div>"
    }
    $lines += "<br><div>$($txt.Closing)</div></div>"
    $mailBody = $lines -join ""
}

# ---- 6. INVIO/VISUALIZZAZIONE ----
